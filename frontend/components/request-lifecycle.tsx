...
export function RequestLifecycle() {
  ...
  const totalPhases = session.steps.length + 2;
  ...
  return (
    <div className="mx-auto w-full max-w-5xl">
      <div className="rounded-t-xl border border-border bg-card/60 px-4 py-2.5 font-mono text-xs backdrop-blur-sm">
        ...
        <span
          className={resolved ? "text-foreground" : "text-muted-foreground/40"}
        >
          {session.model}{" "}
        </span>
        <span className={resolved ? "text-accent" : "text-muted-foreground/40"}>
          {session.status}{" "}
        </span>
        <span
          className={
            resolved ? "text-muted-foreground" : "text-muted-foreground/40"
          }
        >
          {session.latency}
        </span>
      ...
      <div className="rounded-b-xl border border-t-0 border-border bg-card/40 p-5 backdrop-blur-sm md:p-7">
                ...
                return (
                  <span
                    ...
                    className={[
                      ...
                      isActive
                        ? "scale-105 shadow-[0_0_20px_-4px] shadow-primary/50"
                        : "",
                    ].join(" ")}
                  >
                    <span
        ...
        <div className="mt-6 flex items-center justify-between border-t border-border pt-4 font-mono text-xs">
          <span
            className={
              resolved ? "text-foreground" : "text-muted-foreground/40"
            }
          >
